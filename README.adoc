...
If you update the REST service, the changes will be automatically picked up by the Open Liberty server. Your webpage will have an updated table output that responds with the [hotspot]`artists.json` in `src/resources`. You can also add/delete data to cater your desired artist data. You will also have to update or add `Headers` and `accessors` in the [hotspot=table-info]`columns` constant to correspond with the data.
...
Now, you will need to update the [hotspot=accessor-ID]`accessor` in the [hotspot=table-info]`columns` constant.
