...
The [hotspot=state-object file=0]`state` object is initialized to represent the state of the posts that will appear on the paginated table. The `Fetchdata` component also needs to be [hotspot=export-fetchdata file=0]`exported` as a reusable UI element that can be used across your application, such as in the [hotspot=import-Fetchdata file=1]`App.js` file. The `App.js` file has been provided for you, and it returns the `Fetchdata` component as a [hotspot=react-fragment file=1]`React Fragment` to create a reusable element that encompasses your whole web application. 
...
The [hotspot=axios-library]`axios` library is provided in the `Fetchdata` Component.
...
The [hotspot=get-posts]`getPosts()` is the function that will utilize the axios API. The 
[hotspot=axios]`axios` HTTP call is used to read the artist JSON containing the data from the REST endpoint. The loading state is changed by using the [hotspot=then-method]`.then()` method 
to display the data. On successfully receiving a response, the state of 
the system changes by assigning [hotspot=data]`response.data` to [hotspot=data]`posts`. Since axios is promise-based, you can still use the [hotspot=error-handling]`.catch()` method that is used for 
...
=== Displaying the returned data
You need to verify that the component was updated properly. This is done through the [hotspot=mount-posts]`componentDidMount()` lifecycle method, and then displays the returned artist data using the [hotspot=render-posts]`render()` method. Once the state is updated, your component will mount all the children elements and component instances to the native UI. You will now have access to the native UI, and will be able to access to your children references, with the ability to trigger a new render pass. You will use the concept of pagination to display the returned data. Pagination is the process of separating content into discrete pages, and it can be used for handling data sets in React. In your application, you will render the columns in the paginated table. 

The [hotspot=react-table]`react table` libraries are already provided in your component for the paginated table.
...
// =================================================================================================
// Showcasing the JAX-RS API
// =================================================================================================

== Optional: Updating the REST service

If you update the REST service, the changes will be automatically picked up by the Open Liberty server. Your webpage will have an updated table output that responds with the [hotspot file=0]`artists.json` in `src/resources`. You can also add/delete data to cater your desired artist data. In this case, you will also have to update or add `Headers` and `accessors` in the [hotspot=table-info file=1]`columns` constant to correspond with the data.
...
[role="code_command hotspot file=0", subs="quotes"]
----
#Update the `artists.json` file.#
`src/main/resources/artists.json`
----
[role="edit_command_text"]
Change the `"id"` object to `"popularity"` and define an arbitrary numerical value for each artist. For instance `"popularity": "123k"`.

Now, you will need to update the [hotspot=accessor-ID file=1]`accessor` in the [hotspot=table-info file=1]`columns` constant.
...
----
[role="edit_command_text"]
Change [hotspot=accessor-ID]`accessor` to `accessor: "popularity"`.

Rebuild the frontend by running the Maven `generate-resources` goal:
[role='command']
----
mvn generate-resources
----
...
After you start the service, you can find your updated artist JSON at the
http://localhost:9080/artists[http://localhost:9080/artists^] URL.

Navigate to the http://localhost:9080[http://localhost:9080^] URL to view your updated frontend application.
