...
The [hotspot=axios-library]`axios` Library is provided the `Fetchdata` Component.

Fetchdata.js
[source, javascript, linenums, role='code_column hide_tags=react-table,convert-data,mount-posts,render-posts']
----
include::finish/src/main/frontend/src/Components/Fetchdata.js[]
----

The [hotspot=get-posts]`getPosts()` is the function that will utilize the axios API. The 
[hotspot=axios]`axios` HTTP call is used to read the artist JSON containing the data from `artists.json`. The loading state is changed by using the [hotspot=then-method]`.then()` method 
...
=== Displaying the returned data
In this section, you will verify that the component was updated properly through the [hotspot=mount-posts]`componentDidMount()` lifecycle method, and then display the returned artist data using the [hotspot=render-posts]`render()` method in your component. Once the state is updated, your component will mount all the children elements and component instances to the native UI. You will now have access to the native UI, and will be able to access to your children references and the ability to trigger a new render pass. You will use the concept of pagination to display the returned data. Pagination is the process of separating content into discrete pages, and it can be used for handling data sets in React. In your application, you will render the columns in the paginated table. 

The [hotspot=react-table]`react table` libraries are provided for the paginated table.
