...
----
[role="edit_command_text"]
To run the default test, you can add the [hotspot=node-tests file=1]`testing` configuration to the `frontend-maven-plugin` and rerun the Maven `process-resources` goal. You can rebuild the front end
at any time with the Maven `process-resources` goal to pick up new test cases and re-run your tests.
