...
If you update the REST service, the changes will be automatically picked up by the Open Liberty server. Your webpage will have an updated table output that responds with the [hotspot]`artists.json` in `src/resources`. You can also add/delete data to cater your desired artist data. You will have to update or add `Headers` and `accessors` in the [hotspot=table-info]`columns` constant.
...
Change the `"id"` object to `popularity` and define an arbitrary numerical value. For instance `popularity: "123k"`.
...
----
[role="edit_command_text"]
Change [hotspot=accessor-ID]`accessors` to `accessor: "popularity"`.
