...
Likewise, navigate to http://localhost:3000 to view the consumed JSON being displayed using ReactJS and see the following output:
...
Also, pressing CTRL+C in the shell session to deploy the frontend application will stop the node server. 
...
== Displaying the returned data
In this section, we will verify that the component was updated properly and display the returned data. Once the state is updated, your component will mount all the children elements and component instances to the native UI. You will now have access to the native UI, and will be able to access to your children references and the ability to trigger a new render pass. You will use the concept of pagination to display the returned data. Pagination is the process of separating content into discrete pages, and it can be used for handling data sets in React. In your application, you will render the columns in the paginated table. 
...
Whenever you change and build your React implementation, the frontend application root at http://localhost:3000 reflects the changes automatically. You can visit the root to manually check whether the artist JSON was consumed correctly. You can rebuild only your React frontend by running the following command:
...
When you are done checking the application root, exit development mode by pressing CTRL+C in the shell session where you ran the server, or by typing q and then pressing the enter/return key. Likewise, pressing CTRL+C in the shell session to deploy the frontend application will stop the node server. 
