...
You will implement a ReactJS client that consumes this JSON and displays its contents at the following URL: `http://localhost:9080`.
...
Navigate your browser to the application root http://localhost:9080/artists[http://localhost:9080^].
...
The [hotspot=mount-posts]`componentDidMount()` lifecycle method is called once all the children elements and component instances are mounted onto the native UI. You will now have access to the native UI, access to your children references and the ability to trigger a new render pass. Pagination is the process of separating content into discrete pages, and it can be used as for handling data sets in React. In your application, you will render the columns in the paginated table, and define the [hotspot=header-id hotspot=header-name hotspot=header-followers hotspot=header-genres]`Header`
