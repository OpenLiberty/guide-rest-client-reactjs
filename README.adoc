...
The [hotspot=react-library file=1]`React` library imports the `react` package for you to create the [hotspot=ArtistTable file=1]`ArtistTable` function needs to have the `export` declaration as it is being exported to the `App.js` module. The [hotspot=posts file=1]`posts` object is initialized using a React Hook which lets you add a state to represent the state of the posts that appear on the paginated table.
...
The [hotspot=useTable file=1]`useTable` hook is used to create a paginated table. The `useTable` hook takes in the [hotspot=table-info file=1]`columns` and [hotspot=posts file=1]`posts` and `setPosts` objects as parameters. The `useTable` hook returns a paginated table that is assigned to the [hotspot=table file=1]`table` constant. The [hotspot=table file=1]`table` constant is used to render the paginated table on the webpage. The [hotspot=return-table file=1]`return` statement returns the paginated table. The [hotspot=useSortBy file=1]`useSortBy` hook is used to sort the paginated table by the column headers. The [hotspot=usePagination file=1]`usePagination` hook is used to create the pagination buttons that are used to navigate through the paginated table at the bottom of the table.
...
The [hotspot=axios]`axios` HTTP call is used to read the artist JSON that contains the data from the sample JSON file in the `resources` directory. When a response is successful, the state of the system changes by assigning [hotspot=response-data]`response.data` to [hotspot=posts]`posts`. The [hotspot=convert-data]`convertData` function manipulates the JSON data to allow it to be accessed by the `ReactTable`. To learn more about it, see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Spread_syntax#Spread_in_object_literals[Spread in object literals^].
