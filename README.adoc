...
Fetchdata.js
[source, javascript, linenums, role='code_column hide_tags=axios-library,react-table,columns,get-posts,mount-posts,render-posts']
----
...
The [hotspot=mount-posts]`componentDidMount()` lifecycle method is called once all the children elements and component instances are mounted onto the native UI. You will now have access to the native UI, access to your children references and the ability to trigger a new render pass. Pagination is the process of separating content into discrete pages, and it can be used for handling data sets in React. In your application, you will render the columns in the paginated table. 

The [hotspot=columns]`columns` constant is used to define the [hotspot=header-id hotspot=header-name hotspot=header-followers hotspot=header-genres]`Header`
...
Import the [hotspot=react-table]`react table` libraries needed for the paginated table.
...
----
[role="edit_command_text"]
Add the [hotspot=mount-posts]`componentDidMount()` method to your component.
Next add the [hotspot=columns]`columns` variable. Lastly, Add the 
