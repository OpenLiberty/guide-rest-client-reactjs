...
You will need to create the entry point of your React application. In production, when the frontend application is used by other applications, the `index.js` file should be the main entry point of the application as it coincides with the `index.html` file. This is where your code is always going to be run. 
...
The [hotspot=axios]`axios` HTTP call to read the artist JSON
containing the data from the sample JSON files. Then, the
the loading state is changed by using the [hotspot=then-method]`.then()` method 
