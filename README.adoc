...
Update the component to add [hotspot=state-object file=0]`state` object.
...
The [hotspot=mount-posts]`componentDidMount()` lifecycle method is called once all the children elements and component instances are mounted onto the native UI. You will now have access to the native UI, and will be able to access to your children references and the ability to trigger a new render pass. Pagination is the process of separating content into discrete pages, and it can be used for handling data sets in React. In your application, you will render the columns in the paginated table. 
...
----
[role="edit_command_text"]
Add the [hotspot=mount-posts]`componentDidMount()` and [hotspot=render-posts]`render()` 
method to your component. Next add the [hotspot=columns-info]`columns` variable. 
Lastly, add the [hotspot=return-table]`return` statement to add the paginated table to your application.
