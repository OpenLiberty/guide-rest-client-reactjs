...
The React application in this guide is provided and configured for you in the `src/main/frontend` directory. The application uses the one of the https://react.dev/learn/start-a-new-react-project[React-powered frameworks] to set up the modern React application. The https://nextjs.org/[Next.js] framework provides a powerful environment for learning and building React applications, with features like server-side rendering, static site generation, and easy API routes. It is the best way to start building a new, highly performant React application.
...
The `page.jsx` file will be act as a container for all other components. When React component [hotspot=createHome file=0]`Home` is rendered, the [hotspot=displayArtistTable file=0]`ArtistTable` components content will be displayed.
...
For more detailed information on structuring `Pages` and `Layouts` in `Next.js`, please refer to the `Next.js` documentation on https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts#layouts[Pages and Layouts]

// =================================================================================================
// Creating ArtistTable React component
// =================================================================================================
...
`src/main/frontend/src/Components/ArtistTable.jsx`
----
...
The [hotspot=useReactTable file=1]`useReactTable` hook creates a table instance. The hook takes in the [hotspot=table-info file=1]`columns`, [hotspot=posts file=1]`posts` as parameters. The [hotspot=getCoreRowModel file=1]`getCoreRowModel` function is included for the generation of the core row model of the table, which serves as the foundational row model upon pagination and sorting build. The [hotspot=getPaginationRowModel file=1]`getPaginationRowModel` function applies pagination to the core row model, returning a row model that includes only the rows that should be displayed on the current page based on the pagination state. In addition, the [hotspot=getSortedRowModel file=1]`getSortedRowModel` function sorts the paginated table by the column headers then applies the changes to the row model. It returns a paginated table instance that is assigned to the [hotspot=table file=1]`table` constant. The [hotspot=table file=1]`table` constant renders the paginated table on the webpage. The [hotspot=return-table file=1]`return` statement returns the paginated table. 
...
`Next.js` supports a variety of testing tools. This guide uses `Vitest` for unit testing the React components, with the test file [hotspot file]`App.test.jsx` located in `src/main/frontend/__tests__/` directory. The `App.test.jsx` file is a simple JavaScript file that tests against the `page.jsx` component. There are no explicit test cases that are written for this application. To learn more about `Vitest`, please visit https://nextjs.org/docs/app/building-your-application/testing/vitest[Setting up Vitest with Next.js]
